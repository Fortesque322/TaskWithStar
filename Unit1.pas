...
procedure TServer.SaveRecordServer;
  ...
  begin
    ...
    buf:=Self.AsBytes;
    BlockWrite(ByteFileServer, buf, Length(buf));
...
procedure TServer.SetBytes(buf: TBytes);
  ...
  begin
    ...
    BlockRead(ByteFileServer,data,filesize(ByteFileServer));
    SetLength(data,filesize(ByteFileServer));

    Move(data[Counter],size, SizeOF(integer));   // Неверный счетчик
    inc(Counter,SizeOf(integer));

//    SetLength(FModel, size);              // Неверное задание границ
//    Move(buf[Counter],FModel[0],Size);    //Неправильно вписываю данные в массив
...
function TServiceCentr.AsBytes: TBytes;
  ...
  begin
    ...
    buf:=Self.AsBytes;
    BlockWrite(ByteFileService, buf, Length(buf));
...
procedure TServiceCentr.SetBytes(buf: TBytes);
  ...
  begin
    ...
    FileMode:= fmOpenRead;
    Reset(ByteFileService,1);
    BlockRead(ByteFileService,data,filesize(ByteFileService));
    SetLength(data,filesize(ByteFileService));

//    Move(buf[Counter],size, SizeOF(integer));
//    inc(Counter,SizeOf(integer));

    SetLength(FDetails, size);
    Move(buf[Counter],FDetails,Size);
    inc(Counter,Length(FDetails));

    Move(buf[Counter],FDeliveryTimeDay, SizeOF(FDeliveryTimeDay));
    inc(Counter,SizeOf(FDeliveryTimeDay));

    Move(buf[Counter],FBill, SizeOF(FBill));
    close(ByteFileService);
  end;
...
procedure TServiceEngineer.SetBytes(buf: TBytes);
  ...
  begin
    ...
    Move(buf[Counter],FName,Size);
    inc(Counter,Length(FName));

    Move(buf[Counter],FSalary, SizeOF(FSalary));
    inc(Counter,SizeOf(FSalary));

    Move(buf[Counter],FExperience, SizeOF(FExperience));
    close(ByteFileEngineer);
  end;
